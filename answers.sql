...
SELECT models.name, brand_name, headquarters FROM models JOIN brands ON (brands.name = brand_name) WHERE models.name = 'Mustang';
...
SELECT * FROM brands WHERE founded < 1903 ORDER BY founded ASC LIMIT 3; 
...
SELECT COUNT(brand_name) AS count FROM models WHERE brand_name = 'Ford';
...
SELECT models.name, brand_name, founded FROM models LEFT JOIN brands ON (brand_name = brands.name) WHERE models.year = 1960;
...
SELECT brands.name, founded, models.name FROM brands LEFT JOIN models ON (brands.name = brand_name) WHERE discontinued IS NULL;
...
SELECT models.name, brand_name, founded FROM models INNER JOIN brands ON (brands.name = brand_name);
...
SELECT brands.name, founded FROM brands WHERE founded = 2003;
...
SELECT brands.name, models.name, models.year, brands.discontinued, (brands.discontinued - models.year) AS years_until_brand_discontinued FROM models LEFT JOIN brands ON (brand_name = brands.name) WHERE brands.discontinued IS NOT NULL ORDER BY brands.name, models.name, models.year;
...
SELECT brand_name FROM models GROUP BY brand_name HAVING COUNT(models.name) > 5;
...
SELECT models.name, models.year FROM models WHERE models.year = ANY (SELECT brands.founded FROM brands);